...
def train_models(df):
    ...
    try:
        ...
        model_objects["ARIMA"] = m
        joblib.dump(m, os.path.join(MODEL_DIR, "arima.pkl"))
    ...
    try:
        ...
        model_objects["ARIMAX"] = m
        joblib.dump(m, os.path.join(MODEL_DIR, "arimax.pkl"))
    ...
    try:
        ...
        m = ProphetForecaster()
        m.fit(train_df)
        # Prophet expects horizon, not a test_df
        horizon = len(test_df)
        pred_dict = m.predict(horizon)
        # align: pred_dict["predicted"] is forecast from train_df end
        # we compare with test_df["rate"]
        prophet_pred = {
        ...
        model_objects["Prophet"] = m
        joblib.dump(m, os.path.join(MODEL_DIR, "prophet.pkl"))
    ...
    try:
        ...
        model_objects["LSTM"] = m
        # Save LSTM (special handling because of Keras model)
        m.save(os.path.join(MODEL_DIR, "lstm_model"))
        joblib.dump(m, os.path.join(MODEL_DIR, "lstm.pkl"))  # for metadata
    ...
    if best_model:
        # For LSTM, the pickle already saved; we can symlink or copy the saved directory
        if best_name == "LSTM":
            # just copy the entire directory (simplistic)
            import shutil
        ...
        logger.info(f"Best model '{best_name}' saved to best_model.pkl/best_model/")
    else:
        logger.warning(f"Best model object for {best_name} not found")
