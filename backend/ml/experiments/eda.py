"""
eda.py — Comprehensive Exploratory Data Analysis for MWK/USD Exchange Rate
==========================================================================
Adds deep time‑series analysis, modelling, diagnostics, feature
engineering, calendar/regime analysis, and automated reporting.
"""
...
# Optional dependencies — skip gracefully if missing
try:
...
# ──────────────── Original EDA plots (kept for backward compatibility) ────────
def plot_time_series(df: pd.DataFrame) -> None:
    fig, ax = plt.subplots(figsize=(14, 5))
    fig.patch.set_facecolor("white")
    ax.fill_between(df["date"], df["rate"], alpha=0.12, color=PALETTE["band"])
    ax.plot(df["date"], df["rate"], color=PALETTE["rate"], linewidth=1.4, label="MWK/USD Rate")
    ...
    for idx, va, label in [(idx_min, "top", "Min"), (idx_max, "bottom", "Max")]:
        ax.annotate(
            ...
            textcoords="offset points", fontsize=8.5, color="#374151",
...
def plot_rolling_stats(df: pd.DataFrame) -> None:
    ...
    ax_mean, ax_std = axes
    for ax in axes:
        ax.plot(df["date"], df["rate"], color=PALETTE["rate"], alpha=0.20, linewidth=0.8, label="Raw rate")
...
def plot_heatmap(df: pd.DataFrame) -> None:
    ...
    corr = sub.corr()
    label_map = {
        "rate": "Exchange Rate", "Inflation": "Inflation (MW)",
        "Money_Supply": "Money Supply", "Foreign_Reserves": "FX Reserves",
        "Current_Account_Balance": "Current Acct Bal",
        "Lending_Interest_Rate": "Lending Rate",
        "Real_Interest_Rate": "Real Interest Rate",
        "GDP_Growth": "GDP Growth", "us_cpi": "US CPI",
...
# ══════════════════════════════════════════════════════════════════════════════
# 2. DEEPER TIME SERIES ANALYSIS
# ══════════════════════════════════════════════════════════════════════════════
def run_adf_test(df: pd.DataFrame) -> None:
    """Augmented Dickey-Fuller test on exchange rate level and first difference."""
...
def forecast_arima(df: pd.DataFrame, forecast_steps: int = 90) -> None:
    ...
    plt.close(fig)
    print(f"  [✓] Saved  {out}")
    # Print last value and forecast end
...
def forecast_prophet(df: pd.DataFrame) -> None:
    ...
    model = Prophet(daily_seasonality=False, yearly_seasonality=True, changepoint_prior_scale=0.5)
...
def compare_model_importances(df: pd.DataFrame) -> None:
    """RandomForest vs XGBoost vs LightGBM feature importance comparison."""
    FEATURE_COLS = ["Inflation", "Money_Supply", "Foreign_Reserves",
                    "Current_Account_Balance", "Lending_Interest_Rate",
                    "Real_Interest_Rate", "GDP_Growth", "us_cpi",
                    "us_fed_rate", "inflation_diff", "interest_rate_diff"]
    feat_cols = [c for c in FEATURE_COLS if c in df.columns]
...
def walk_forward_cv(df: pd.DataFrame, n_splits: int = 5) -> None:
    """Walk-forward cross-validation with RandomForest; reports RMSE/MAE."""
    FEATURE_COLS = ["Inflation", "Money_Supply", "Foreign_Reserves",
                    "Current_Account_Balance", "Lending_Interest_Rate",
                    "Real_Interest_Rate", "GDP_Growth", "us_cpi",
                    "us_fed_rate", "inflation_diff", "interest_rate_diff",
    ...
    # Save scores to CSV
    pd.DataFrame({"fold": range(1, n_splits+1), "RMSE": rmse_list, "MAE": mae_list}).to_csv(
        os.path.join(REPORT_DIR, "walk_forward_cv.csv"), index=False)
...
def outlier_detection(df: pd.DataFrame) -> None:
    ...
    ax.scatter(df["date"][outliers_mask], df["rate"][outliers_mask],
...
def plot_distributions(df: pd.DataFrame) -> None:
    """KDE + histogram for each numeric column (excluding date)."""
    ...
    for i, col in enumerate(numeric_cols):
        sns.histplot(df[col].dropna(), kde=True, ax=axes[i], color=PALETTE["mean7"],
...
def granger_causality(df: pd.DataFrame, maxlag: int = 7) -> None:
    """Test if inflation (lagged) Granger‑causes exchange rate changes."""
    ...
    result = grangercausalitytests(data[["rate_diff", "inflation"]], maxlag=maxlag, verbose=False)
...
def plot_lag_correlation(df: pd.DataFrame, max_lag: int = 30) -> None:
    """Correlation between rate and each macro feature at different lags."""
...
def mutual_information_scores(df: pd.DataFrame) -> None:
    """Compute mutual information between feature set and rate."""
    FEATURE_COLS = ["Inflation", "Money_Supply", "Foreign_Reserves",
                    "Current_Account_Balance", "Lending_Interest_Rate",
                    "Real_Interest_Rate", "GDP_Growth", "us_cpi",
                    "us_fed_rate", "inflation_diff", "interest_rate_diff"]
...
def dimensionality_reduction(df: pd.DataFrame) -> None:
    """PCA scatter plot coloured by year & rate level, plus optional UMAP."""
    FEATURE_COLS = ["Inflation", "Money_Supply", "Foreign_Reserves",
                    "Current_Account_Balance", "Lending_Interest_Rate",
    ...
    scatter1 = axes[0].scatter(X_pca[:,0], X_pca[:,1], c=years, cmap="viridis",
...
def pegged_vs_nonpegged(df: pd.DataFrame) -> None:
    ...
    df_copy = df.copy()
    df_copy["period"] = np.where(df_copy["is_pegged_regime"]==1, "Pegged", "Non‑pegged")
    sns.boxplot(x="period", y="rate", data=df_copy, palette="Set2", showfliers=False, ax=ax)
...
def year_over_year_heatmap(df: pd.DataFrame) -> None:
    """Heatmap of year‑over‑year rate change (month × year)."""
    df_copy = df.copy()
    df_copy["year"] = df_copy["date"].dt.year
    df_copy["month"] = df_copy["date"].dt.month
    # last available rate per month
    monthly_end = df_copy.groupby(["year", "month"])["rate"].last().unstack(level=0)
    # YoY % change: (rate_current - rate_same_month_prev_year) / rate_same_month_prev_year *100
    pct_change = monthly_end.pct_change(axis=1, periods=12) * 100   # careful: this is not correct for YoY with date index
    # Better: shift columns by year
    ...
    results = results.T   # rows: year, cols: month
...
def export_stats_summary(df: pd.DataFrame) -> None:
    ...
    stats_dict = {
        ...
        "adf_statistic": adfuller(df.set_index("date")["rate"].dropna())[0],
        "adf_pvalue": adfuller(df.set_index("date")["rate"].dropna())[1],
    }
    for col in df.select_dtypes(include=[np.number]).columns:
        stats_dict[f"{col}_mean"] = df[col].mean()
        stats_dict[f"{col}_std"] = df[col].std()
        stats_dict[f"{col}_missing"] = df[col].isnull().sum()
    pd.DataFrame([stats_dict]).to_csv(os.path.join(REPORT_DIR, "statistics_summary.csv"), index=False)
    print(f"  [✓] Statistics summary saved to {REPORT_DIR}/statistics_summary.csv")

def generate_pdf_report() -> None:
    """Combine all saved PNG files into a single PDF report."""
