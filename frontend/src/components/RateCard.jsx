...
export default function RateCard({ latestRate, loading }) {
  if (loading) {
    return (
      <div className="bg-stone-900/60 rounded-2xl border border-stone-700/60 backdrop-blur p-5 animate-pulse">
        <div className="h-4 w-28 bg-stone-700 rounded mb-2" />
        <div className="h-8 w-24 bg-stone-700 rounded" />
        <div className="h-3 w-16 bg-stone-700 rounded mt-2" />
      </div>
    )
  }
  ...
  if (!latestRate || typeof latestRate.rate !== 'number') {
    return (
      <div className="bg-stone-900/60 rounded-2xl border border-stone-700/60 backdrop-blur p-5">
        <p className="text-stone-400 text-sm">No current rate available</p>
      </div>
    )
  }
  ...
  const colorClass =
    direction === 'up' ? 'text-green-400' : direction === 'down' ? 'text-terracotta-400' : 'text-stone-400'
