...
function WhenToAct({ nextDayChange, sevenDayChange, thirtyDayChange }) {
  const getAdvice = (label, change) => {
    ...
    if (Math.abs(pct) < 0.3) return { level: "Stable", advice: "The rate is holding steady. No action needed.", color: "text-emerald-400", bg: "bg-emerald-500/10" };
    if (dir === "up") return { level: "Weakening", advice: "Kwacha losing value. If you need USD, buy now.", color: "text-red-400", bg: "bg-red-500/10" };
    return { level: "Strengthening", advice: "Kwacha gaining. If you hold USD, convert now.", color: "text-emerald-400", bg: "bg-emerald-500/10" };
  };
  const stages = [
        ...
        {stages.map((stage, i) => {
          ...
          return (
            <div key={i} className={`${advice.bg} rounded-xl p-4`}>
              ...
              {stage.change && <p className="text-xs text-slate-500 mt-1">Change: {stage.change.direction === "up" ? "↗" : "↘"} {stage.change.pct}%</p>}
            </div>
          );
        })}
      </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <button onClick={handleGenerate} disabled={generating} className="bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-600 text-white text-sm px-4 py-2 rounded-lg font-medium transition flex items-center gap-2">
          {generating ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <RefreshCw className="w-3.5 h-3.5" />}{generating ? "Generating..." : "Refresh forecasts"}
        </button>
      </div>
